...
const vcard = 'BEGIN:VCARD\n' //GANTI NO DAN NAMA DIBAWAH INI JADI PUNYA LU
            ...
            + 'FN: Fakhri Awail\n' //PENGEMBANG BY :
            + 'ORG: Xzer0 Bot;\n' //MYBOT TEAM
            + 'TEL;type=CELL;type=VOICE;waid=6283898935905:+62 838-9893-5905\n' 
...
/******** OWNER NUMBER**********/
const ownerNumber = ["6283898935905@s.whatsapp.net","6283898935905@s.whatsapp.net"] //ganti nomor lu
/************************************/
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ *FADHIL BOT BROADCAST*❯\n\n${body.slice(4)}`})
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 *FADHIL BOT BROADCAST* 」*\n\n${body.slice(4)}`)
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					}
					break
